...
export default function Catalogue(props) {
    ...
    const [voiceState, setVoiceState] = useState('start');
    ...
    const handleSpeak = () => {
        if (voiceState === 'start') {
            Voice.start(Enum.defaultVoice, Enum.voiceParam);
        }
        if (voiceState === 'stop') {
            Voice.stop();
        }
        setVoiceState(voiceState === 'start' ? 'stop' : 'start');
    };
    return <View style={[gcss.flexContainer]}>
        <TopAppName {...props}/>
        <Heading val={category}/>
        <View style={gcss.inrow}>
            <CustomInput placeholder={'Search Products'} width={300} onChange={val => setSearchVal(val)}
        ...
        <View style={gcss.inrow}>
            ...
            <AppButton title={voiceState} width={70} onPress={handleSpeak}/>
            {voiceResults.map(x => <Text>{x.value}</Text>)}
        </View>
