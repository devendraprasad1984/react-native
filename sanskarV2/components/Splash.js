...
const Splash = props => {
    ...
    const checkFromLocal = () => {
        console.log('params',params,'signout',signout);
    ...
    const navNextFromSplash = (xinfo) => {
        // console.log({dealerInfo, config});
        props.navigation.navigate(Enum.screenName.home,{dealerInfo:xinfo, config});
    }

    useEffect(() => {
        // checkFromLocal();
        getFromAPI(Config.configEndPoint,data => {
            setsplashtext(configFilterByKey(data, Enum.configKeys.splashtxt));
            setsplashimage(configFilterByKey(data, Enum.configKeys.splashimg));
            setLicence(configFilterByKey(data, Enum.configKeys.licence));
            setTagLine(configFilterByKey(data, Enum.configKeys.tagline));
            setConfig(data);
            setLoad(false);
        });
    }, []);
    ...
    const handleGo = () => {
        ...
        postToAPI(Config.agentValidator, payload, (res) => {
            ...
            if (res.status === Enum.successMsg) {
                ...
                navNextFromSplash(mainDealerInfo)
    ...
    const LoggedInChecker = () => {
        ...
        if (!loggedin && mount) {
            return <View style={[gcss.inrow]}>
                <CustomInput type='password' width={160} placeholder='passcode' onChange={(val) => pin = val}/>
            ...
            return <View style={[gcss.inrow]}>
                <AppButton width={150} onPress={handleGo} title={`hi, ${dealerInfo.agentid}`}/>
                <AppButton width={50} onPress={handleHelp} title='??'/>
            </View>
        }
    }
