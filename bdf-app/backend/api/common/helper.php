...
{
    global $conn, $nodatafound;
    $sql = $conn->query("SELECT cat.*, det.detail_description FROM category cat inner join categorydetails det ON cat.id=det.id");
...
function handleEventRegistrationSave($data)
{
    ...
    $query = "insert into eventregistrationdetails(eventid,name, email, mobile, info) values('$eventid','$name','$email','$mobile','$info')";
